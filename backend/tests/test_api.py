"""
LinkKeep API — Integration Tests
Run: pytest tests/ -v
"""
import pytest
...
def auth_user(client):
    """Register + login, return token + user data."""
...
class TestAuth:
    ...
    def test_login_success(self, client, auth_user):
        # already registered in fixture, test direct login
...
class TestTabs:
    ...
    def test_update_tab(self, client, auth_user):
        ...
        resp = client.put(f"/api/tabs/{tab_id}", json={"name": "New", "color": "#ef4444"}, headers=auth_user)
        assert resp.status_code == 200
...
class TestLinks:
    ...
    def test_create_link_no_tab(self, client, auth_user):
        resp = client.post("/api/links", json={
            "title": "Standalone", "url": "https://example.com"
        }, headers=auth_user)
        assert resp.status_code == 201
