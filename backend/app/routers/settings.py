...
def export_data(user: User = Depends(_get_current_user), db: Session = Depends(get_db)):
    ...
    return {
        ...
        "tabs": [
            {"id": t.id, "name": t.name, "color": t.color, "icon": t.icon, "sort_order": t.sort_order}
            for t in tabs
        ],
        "links": [
            {
                ...
                "is_favorite": l.is_favorite, "sort_order": l.sort_order,
...
def export_html(user: User = Depends(_get_current_user), db: Session = Depends(get_db)):
    ...
    html_parts = [
        ...
        f"<title>LinkKeep Export — {user.username}</title>",
        "</head><body>",
        f"<h1>LinkKeep — {user.username}</h1>",
    ]

    for tab in tabs:
        tab_links = [l for l in links if l.tab_id == tab.id]
        html_parts.append(f"<h2>{tab.name}</h2><ul>")
        for link in tab_links:
            html_parts.append(f'<li><a href="{link.url}">{link.title}</a></li>')
