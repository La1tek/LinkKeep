...
export const api = {
  ...
  rebuildEmbeddings: () => request('/embeddings/rebuild', { method: 'POST' }),
  assistantQuery: (data) => request('/assistant/query', { method: 'POST', body: data }),
  knowledgeGraph: () => request('/knowledge-graph'),
  ...
  snoozeReminder: (linkId, data) => request(`/reminders/${linkId}/snooze`, { method: 'POST', body: data }),
  clearReminder: (linkId) => request(`/reminders/${linkId}`, { method: 'DELETE' }),
  processReminders: () => request('/reminders/process', { method: 'POST' }),
  createDigest: (kind) => request(`/digest/${encodeURIComponent(kind)}`, { method: 'POST' }),
  digestPreview: (kind) => request(`/digest/preview/${encodeURIComponent(kind)}`),
  ...
  listCommands: (q = '') => request(`/commands${q ? `?q=${encodeURIComponent(q)}` : ''}`),
  runCommand: (data) => request('/commands/run', { method: 'POST', body: data }),
